...
function prepareComputedStyleDeclaration(elementImpl, { styleCache }) {
  ...
  const { uaCandidates, customProps } = applyStyleSheetRules(elementImpl, declaration);

  for (let i = 0; i < style.length; i++) {
    handlePropertyForInlineStyle(style.item(i), declaration, style, uaCandidates, customProps);
  }
...
// https://drafts.csswg.org/css-variables-1/#substitute-a-var
// Finds the first top-level comma in a string (not inside nested parens).
...
// Returns the resolved string, or null if any var() is unresolvable (no fallback).
function resolveVarReferences(value, customProps, depth = 0) {
  if (depth > 32 || !value.includes("var(")) {
    return value;
  }
  ...
  while (i < value.length) {
    ...
    const inner = value.slice(varIdx + 4, j - 1);
    const commaIdx = findTopLevelComma(inner);
    let propName, fallback;
    if (commaIdx === -1) {
      propName = inner.trim();
      fallback = null;
    } else {
    ...
    const propValue = propName.startsWith("--") ? customProps.get(propName) : undefined;
    if (propValue !== undefined && propValue !== "") {
      const resolved = resolveVarReferences(propValue, customProps, depth + 1);
      if (resolved !== null) {
        result += resolved;
      } else if (fallback !== null) {
        result += resolveVarReferences(fallback, customProps, depth + 1) ?? fallback;
      } else {
        return null;
      }
    } else if (fallback !== null) {
...
function applyStyleSheetRules(elementImpl, declaration) {
  ...
  // Separate custom properties (--*) from regular ones.
  const customPropCandidates = new Map();
  const regularCandidates = new Map();
  for (const [property, winner] of candidates) {
    if (property.startsWith("--")) {
      customPropCandidates.set(property, winner);
    } else {
      regularCandidates.set(property, winner);
    }
  }

  // Build complete custom property map: inherited from parent, overridden by own cascade winners.
  const customProps = new Map();
  if (elementImpl.parentElement !== null) {
    const parentDecl = getComputedStyleDeclaration(elementImpl.parentElement);
    for (let i = 0; i < parentDecl.length; i++) {
      const prop = parentDecl.item(i);
      if (prop.startsWith("--")) {
        customProps.set(prop, parentDecl.getPropertyValue(prop));
      }
    }
  }
  for (const [property, winner] of customPropCandidates) {
    customProps.set(property, winner.value);
  }

  // Apply all custom properties (own + inherited) to declaration, preserving !important for own winners.
  for (const [property, value] of customProps) {
    const winner = customPropCandidates.get(property);
    declaration.setProperty(property, value, winner?.isImportant ? "important" : "");
  }

  // Apply regular properties, resolving var() references first.
  for (const [property, winner] of regularCandidates) {
    let value = winner.value;
    if (customProps.size > 0 && value.includes("var(")) {
  ...
  return { uaCandidates, customProps };
}
...
function handlePropertyForInlineStyle(property, declaration, style, uaCandidates, customProps) {
